...
### Added
- This change log ([#216])
- Experimental memoizing [parser](._memo-parser_README.md)
...
### Added
- Added an [`ignore`](rules_no-unresolved.md#ignore) option to [`no-unresolved`] for those pesky files that no resolver can find. (still prefer enhancing the Webpack and Node resolvers to using it, though). See [#89] for details.
